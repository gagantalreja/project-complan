...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Command line arguments for extracting Confluence page history into a git repository"
    )
    ...
    parser.add_argument(
        ...
        "--space",
        type=str,
        help="Key of the confluence space",
        required=True,
    )
    parser.add_argument(
        "-pg",
        "--pages",
        type=str,
        help="Title of the pages to extract separated by commas",
        required=True,
    )
...
class AtlassianConverter(MarkdownConverter):
    def __init__(self, **options):
        ...
        setattr(
            self,
            "convert_ac:structured-macro",
            self.convert_ac_structured_macro,
        )
    ...
    def convert_ac_plain_text_body(self, el, text, convert_as_inline):
        if el.parent.attrs["ac:name"] == "code":
            return self.convert_p(
                el,
                self.convert_code(el, text, convert_as_inline),
                convert_as_inline,
            )
        else:
